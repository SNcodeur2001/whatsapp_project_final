  ...
  return createElement('div', {
    ...
    createElement('div', {
      ...
      createElement('div', {
        ...
        createElement('span', { class: ['cursor-pointer'] }, '🖼️'),
...
function createFilterButton({ name, active }) {
  return createElement('button', {
    class: [
      ...
      'text-sm',
      'whitespace-nowrap',
      active ? 'bg-[#e9edef] text-[#111b21]' : 'text-[#54656f] hover:bg-[#e9edef]'
    ]
  }, name);
}
...
function createArchivedSection() {
  return createElement('div', {
    ...
    createElement('span', { class: ['text-[#00a884]'] }, '📦'),
...
function createChatItem(chat) {
  ...
  const sender = store.state.users.find(u => u.id === lastMessage.senderId);
  ...
  return createElement('div', {
    ...
    createElement('div', {
      class: ['w-12', 'h-12', 'rounded-full', 'bg-[#dfe5e7]', 'flex-shrink-0']
    }),
    createElement('div', {
